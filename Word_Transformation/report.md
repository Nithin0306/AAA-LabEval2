...
- `429_1` exhibits strong quadratic growth as V increases (reaching 11.29 ms at V=200). Because every BFS expansion step scans the entire dictionary, per-query search cost scales as `O(V² · L)`.
...
- `429_1` scales linearly and steeply with Q, climbing to **47.07 ms** at Q=200 (over 11× the Q=1 baseline) as each query re-runs a full `O(V² · L)` BFS.
- `429_2` stays flat (~3.90 ms) across all Q values. Initial graph construction `O(V² · L)` is paid once, allowing subsequent queries to execute in `O(V + E)` time using direct array index lookups.
- `429_3` grows moderately to **9.05 ms** at Q=200. Constructing the wildcard map costs only `O(V · L)` upfront, and per-query BFS executes in `O(V · L + E)`, scaling far better than `429_1`.
...
- `429_2` performs `O(V² · L)` pairwise comparisons upfront to build an explicit adjacency list.
- `429_3` builds a wildcard map (`word -> pattern`) in linear time `O(V · L)`, keeping upfront setup cost minimal while avoiding `O(V²)` all-pairs processing.
...
- **`429_1` (On-the-fly Pairwise Scan):**
  - **Best for:** Very small dictionaries (V < 50) or one-off single queries (Q ≈ 1–5).
  - **Trade-off:** Zero memory overhead (O(1) extra space beyond words list), but total execution scales poorly as `O(Q · V² · L)`.

- **`429_2` (Prebuilt Adjacency List):**
  - **Best for:** Scenarios with a high volume of queries (Q ≫ V) on small to medium dictionaries.
  - **Trade-off:** Pays a heavy `O(V² · L)` upfront preprocessing penalty by checking all word pairs. Once built, queries run in optimal `O(V + E)` graph traversal time.

- **`429_3` (Wildcard Pattern Map):**
  - **Best for:** Large dictionaries (V).
  - **Why it excels:** By mapping words to L wildcard buckets (e.g., `h*t -> hot, hat`), `429_3` reduces preprocessing from `O(V² · L)` down to **`O(V · L)`**, completely avoiding `O(V²)` pairwise word comparisons.
  - **Complexity summary:**
    - Preprocessing: `O(V · L)`
    - Per-query BFS: `O(V · L + E)`
    - Total: `O(V · L + Q · (V · L + E))`
